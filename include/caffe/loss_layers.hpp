...
template <typename Dtype>
class P2SLossLayer : public LossLayer<Dtype> {
 public:
  explicit P2SLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "P2SLoss"; }
  //Dtype lamda;
  /**
   * Unlike most loss layers, in the ContrastiveLossLayer we can backpropagate
   * to the first two inputs.
 ...
 protected:
  ...
  Blob<Dtype>  feature_probe_;
  Blob<Dtype>  label_probe_;
  Blob<Dtype>  index_probe_;
  Blob<Dtype>  feature_gallery_;
  Blob<Dtype>  label_gallery_;
  Blob<Dtype>  index_gallery_;
  Blob<Dtype>  diff_ab_;
  Blob<Dtype>  dist_ab_;

  Blob<Dtype>  temp_feature_probe_;
  Blob<Dtype>  temp_feature_gallery_;
  Blob<Dtype>  temp_sign_;
  Blob<Dtype>  temp_dist_ab_;

  Blob<Dtype>  feature_anchor_;
  Blob<Dtype>  label_anchor_;
  Blob<Dtype>  index_anchor_;
  Blob<Dtype>  feature_positive_;
  Blob<Dtype>  label_positive_;
  Blob<Dtype>  index_positive_;
  Blob<Dtype>  feature_negative_;
  Blob<Dtype>  label_negative_;
  Blob<Dtype>  index_negative_;

  Blob<Dtype>  diff_anchor_positive_;
  Blob<Dtype>  diff_anchor_negative_;
  Blob<Dtype>  diff_positive_negative_;
  Blob<Dtype>  dist_anchor_positive_;
  Blob<Dtype>  dist_anchor_negative_;
  Blob<Dtype>  dist_positive_negative_;
...
template <typename Dtype>  
class SymmetricTripletLossLayer : public LossLayer<Dtype> {  
 public:  
  explicit SymmetricTripletLossLayer(const LayerParameter& param)  
      : LossLayer<Dtype>(param){}  
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,  
      const vector<Blob<Dtype>*>& top);  
  
  virtual inline int ExactNumBottomBlobs() const { return 3; }  
  virtual inline const char* type() const { return "SymmetricTripletLoss"; }  
  /** 
   * Unlike most loss layers, in the TripletLossLayer we can backpropagate 
   * to the first three inputs. 
   */  
 ...
 //    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);  
  ...
  Blob<Dtype> diff_a_;            // cached for backward pass  
  Blob<Dtype> diff_p_;            // cached for backward pass  
  Blob<Dtype> diff_n_;            // cached for backward pass  
   
  Blob<Dtype> dist_ap_;          // cached for forward pass  
  Blob<Dtype> dist_an_;          // cached for forward pass
  Blob<Dtype> dist_pn_;          // cached for forward pass    
};
